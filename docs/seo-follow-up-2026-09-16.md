...
## Chiusura del blocco di correzioni
- Completati i testi dei giorni e delle ore marine, popup/tooltip dell'atlante, confronto geografico tra porti, caricamento, errori e avvertenze nelle cinque lingue. Date nella lingua selezionata; nomi dei porti preservati ed escapati nell'HTML. I settori dei popup vengono tradotti al rendering anche quando i dati provengono da una cache creata in un'altra lingua.
- Valori marini nulli/non numerici non vengono più trasformati in onde nulle o temperature zero. L'indice corrente viene nascosto se mancano onde o corrente; i punti senza dati sono grigi. Il confronto esclude i campioni mancanti, segnala la copertura parziale e mantiene l'indice geografico originale del campione peggiore.
- Cache corrotta del confronto ignorata con recupero dal provider; cache valida riutilizzata. In caso di errore viene nascosta la vecchia mappa del confronto. Ripristinato il pulsante di riprova dell'atlante quando manca la configurazione del paese.
- Storico: anche la massima odierna assente non viene interpretata come zero. Corrette le frasi residue osservate nella home francese (introduzione, installazione e qualità dell'aria).
- Verifica finale: 19 suite automatiche superate; sintassi e diff controllati. Browser locale: confronto Messina–Napoli riuscito in francese e spagnolo, cambio giorno marino riuscito; nessun errore console nel campione francese. Test deterministici coprono cinque lingue, dati assenti/parziali, indice del campione, popup, escaping, errori e cache valida/corrotta.
- Completato il blocco di correzioni concordato per la PR. Non equivale a una certificazione di ogni frase/stato dell'intera applicazione né a una garanzia di posizionamento. La diagnosi LCP non ha prodotto una modifica prestazionale dimostrata; la misurazione dopo pubblicazione e l'evoluzione dei report Google restano verifiche successive, non risultati già ottenuti. Nessun merge su main, nessun invio manuale a Search Console.
