...
function groupPlaces(places) {
  ...
  for (const place of places) {
    if (!countries.has(place.cc)) countries.set(place.cc, { name: place.c, code: place.cc, regions: new Map() });
  ...
  return [...countries.values()]
    ...
    .map(country => ({
      ...
      regions: [...country.regions.entries()]
        .map(([name, entries]) => ({ name, entries: entries.sort((left, right) => right.p - left.p || left.n.localeCompare(right.n, 'it')) }))
        .sort((left, right) => left.name.localeCompare(right.name, 'it'))
    }));
}
...
module.exports = async function handler(_req, res) {
  ...
  const countries = groupPlaces(places);
  const canonical = `${SITE_ORIGIN}/localita`;
  const title = 'Località meteo attive: previsioni per città e regioni | Meteo AI';
  const description = `Consulta le previsioni Meteo AI per ${places.length} località attive, organizzate per paese e regione. La directory cresce soltanto con le località realmente cercate dagli utenti.`;
    ...
    '@graph': [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Località meteo', item: canonical }
        ]
      }
    ]
  };
  ...
  const countrySections = countries.map(country => `
    <section class="country" id="paese-${slug(country.code)}">
      <h2>Meteo in ${escapeHtml(country.name)}</h2>
      ${country.regions.map(region => `
        <section class="region" id="regione-${slug(country.code)}-${slug(region.name)}">
          <h3>${escapeHtml(region.name)}</h3>
          <div class="places">${region.entries.map(place => `<a href="${escapeHtml(place.path)}"><strong>Meteo ${escapeHtml(place.n)}</strong><small>Oggi, domani e 7 giorni</small></a>`).join('')}</div>
        </section>`).join('')}
    </section>`).join('');
